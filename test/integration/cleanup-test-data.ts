...
Examples:
  ...
  if (options.directories.length === 0) {
    console.error("❌ Error: at least one output directory is required");
...
async function deleteWorkoutEntity(
  ...
  try {
    ...
    if (result.statusCode && result.statusCode >= 400) {
      ...
      if (verbose) {
        console.info(`  ❌ ${entity.id}: ${errorMsg}`);
      }
    ...
    if (verbose) {
      console.info(
        `  ✅ ${entity.id} (Pinecone: ${body?.pineconeCleanup ? "cleaned" : "skipped"})`,
      );
    }
...
async function deleteProgramEntity(
  ...
  try {
    ...
    if (verbose) {
      console.info(`  ✅ ${entity.id} (hard deleted)`);
    }
...
function displayEntities(entities: EntityToDelete[]) {
  ...
  if (workouts.length > 0) {
    console.info(`📋 Workouts (${workouts.length}):`);
  ...
  if (programs.length > 0) {
    console.info(`📋 Programs (${programs.length}):`);
...
async function main() {
  console.info("🧹 Test Data Cleanup Script\n");
  ...
  if (entities.length === 0) {
    console.info(
      "\n✅ No entities found in result files. Nothing to clean up.\n",
    );
    return;
  }
  ...
  // 3. Dry run check
  if (options.dryRun) {
    console.info("🔍 DRY RUN: No entities were deleted");
  ...
  if (!options.autoConfirm) {
    const confirmed = await confirm(
      `⚠️  Are you sure you want to permanently delete ${entities.length} entities? (y/N): `,
    );
    if (!confirmed) {
  ...
  console.info("\n═══════════════════════════════════════════════════════════");
  console.info(`✅ Successfully deleted: ${succeeded}`);
  if (failed > 0) {
    console.info(`❌ Failed to delete: ${failed}`);
    console.info("\nFailures:");
    results
      .filter((r) => !r.success)
      .forEach((r) => {
