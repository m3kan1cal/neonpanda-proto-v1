...
function extractMetadata(content, filename) {
  ...
  for (const d of disciplineKeywords) {
    if (
      contentLower.includes(d) ||
      filenameLower.includes(d.replace(" ", ""))
    ) {
      discipline = d;
      break;
    }
  ...
  for (const { level: lvl, keywords } of levelMap) {
    for (const k of keywords) {
      if (
        ...
        filenameLower.includes(k.replace(" ", ""))
      ) {
        level = lvl;
        break;
      }
  ...
  for (const keyword of topicKeywords) {
    if (
      ...
      filenameLower.includes(keyword.replace(" ", ""))
    ) {
...
async function upsertAllMethodologies() {
  try {
    ...
    console.info(`   Namespace: ${NAMESPACE}`);
    console.info(
      `   API Key set: ${process.env.PINECONE_API_KEY ? "Yes" : "No (using fallback!)"}`,
    );
    ...
    for (const file of files) {
      try {
        ...
        // Upsert to Pinecone. Same signature as api-helpers storeContextInPinecone:
        // upsertRecords({ records: [...] }). Record shape (id, text, ...metadata) is
        // unchanged — only the call format was fixed; query path expects hit.fields.text + hit.fields.
        await index.namespace(NAMESPACE).upsertRecords({
          records: [
            {
              id: recordId,
              text: content, // Auto-embedded; returned as hit.fields.text by searchRecords
              ...metadata,
            },
          ],
        });
