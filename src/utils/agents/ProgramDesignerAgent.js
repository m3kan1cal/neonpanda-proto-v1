 ...
 * This class manages the program design flow, conversation state, and API interactions
...
export class ProgramDesignerAgent {
  constructor(options = {}) {
    ...
    this.sessionId = options.sessionId || null; // Program designer session ID
    ...
    this.state = {
      ...
      // Progress tracking for program design
  ...
  /**
   * Loads an existing program designer session and its conversation history
   * Transforms session.conversationHistory to state.messages for UI display
   */
  async loadSession(userId, sessionId) {
    ...
    try {
      ...
      const result = await getProgramDesignerSession(userId, sessionId);
      ...
      this.coachId = session.coachId;

      // Transform conversationHistory to messages for UI
      const messages = (session.conversationHistory || []).map(
        (msg, index) => ({
          ...
          imageS3Keys: msg.imageS3Keys || [],
        }),
      );

      // Check if session is already complete (program already generated)
      const isSessionComplete = session.isComplete === true;

      // Update state with messages, progress, and completion status
      this._updateState({
        messages,
        isLoadingItem: false,
  ...
  async loadExistingConversation(userId, coachId, conversationId) {
    ...
    try {
      ...
      const [conversationData] = await Promise.all([
        getProgramDesignerConversation(userId, coachId, conversationId),
  ...
  async sendMessageStream(messageContent, imageS3Keys = []) {
    ...
    try {
      ...
      try {
        // Get streaming response (API layer handles Lambda streaming for program design)
        const messageStream = streamProgramDesign(
          this.userId,
          this.sessionId,
          messageContent.trim(),
          imageS3Keys,
        ...
        return await processStreamingChunks(messageStream, {
          ...
          onMetadata: async (metadata) => {
            ...
            if (metadata.progress) {
              this._updateState({
                progress: {
                  questionsCompleted:
                    metadata.progress.questionsCompleted ??
                    metadata.progress.itemsCompleted ??
                    metadata.progress.completed ??
                    0,
                  estimatedTotal:
                    metadata.progress.estimatedTotal ??
                    metadata.progress.totalItems ??
          ...
          onComplete: async (chunk) => {
            ...
            // Check if program generation was triggered (session complete)
            const programGenerationTriggered =
              chunk.metadata?.programGenerationTriggered === true;
            const isSessionComplete = chunk.isComplete === true;
          ...
          onFallback: async (data) => {
            ...
            // Explicitly set mode AFTER object creation if provided by backend
            if (updatedConversation && data?.hasOwnProperty("mode")) {
  ...
  async updateCoachConversation(userId, coachId, conversationId, metadata) {
    ...
    try {
      ...
      const result = await updateProgramDesignerConversation(
        userId,
        coachId,
        conversationId,
        metadata,
  ...
  async deleteCoachConversation(userId, coachId, conversationId) {
    ...
    try {
      ...
      const result = await deleteProgramDesignerConversation(
        userId,
        coachId,
        conversationId,
      );
  ...
  /**
   * Handles program completion and redirects to manage programs page
  ...
  handleProgramComplete(programId) {
    ...
    // Navigate to manage programs page to see the new program
    if (this.onNavigation) {
      this.onNavigation("program-complete", {
        userId: this.userId,
        coachId: this.coachId,
        programId,
      });
   ...
   */
  handleCompletion() {
    // Session completion is tracked in DynamoDB
    ...
    // Notify navigation handler (triggers completion modal)
    if (typeof this.onNavigation === "function") {
...
export default ProgramDesignerAgent;
