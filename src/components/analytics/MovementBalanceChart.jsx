...
export default function MovementBalanceChart({
  ...
  return (
    <ChartCard
      ...
      subtitle="Volume by movement pattern — current week vs average"
      isLoading={isLoading}
      isEmpty={!hasData}
      ...
      <div className="w-full" style={{ height: 280 }}>
        <ResponsiveContainer width="100%" height="100%">
          <RadarChart data={radarData} cx="50%" cy="50%" outerRadius="70%">
            ...
            {/* Average (prior weeks) — rendered first so current overlaps */}
            <Radar
    ...
    >
      ...
      {payload.map((entry, i) => (
        <p key={i} className="font-body text-xs" style={{ color: entry.color }}>
          {entry.name}: <span className="font-semibold">{entry.value.toLocaleString()} lbs</span>
        </p>
      ))}
    </div>
  );
