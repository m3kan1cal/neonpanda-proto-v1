...
import TrainingProgramCalendar from './TrainingProgramCalendar';
import PhaseTimeline from './PhaseTimeline';
import PhaseBreakdown from './PhaseBreakdown';

export default function TrainingProgramDashboard() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (!programAgentRef.current) {
        programAgentRef.current = new TrainingProgramAgent(
          userId,
          coachId,
          (newState) => {
      ...
      const programData = await programAgentRef.current.loadTrainingProgram(programId);
  ...
  if (error) {
    return (
      <CenteredErrorState
        ...
        onButtonClick={() => navigate(`/training-grounds/training-programs?userId=${userId}&coachId=${coachId}`)}
        variant="error"
      />
    );
  }
