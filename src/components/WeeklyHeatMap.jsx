...
const HeatMapSquare = ({ dayData, userId, coachId, date }) => {
  ...
  const handleClick = () => {
    if (dayData?.primary_workout_id) {
      navigate(
        `/training-grounds/workouts?workoutId=${dayData.primary_workout_id}&userId=${userId}&coachId=${coachId}`,
      );
    }
  };
  ...
  const getTooltipContent = () => {
    ...
    const rpeText = dayData.avg_rpe
      ? `RPE ${dayData.avg_rpe.toFixed(1)}`
      : "No RPE";
    const intensityText = dayData.avg_intensity
      ? `Intensity ${dayData.avg_intensity.toFixed(1)}`
      : "No Intensity";
  ...
  return (
    ...
    >
      ...
      {hasWorkout && <div className={heatMapPatterns.glowOverlay}></div>}
    </div>
  );
};
...
const HeatMapLegend = () => {
  const legendItems = [
    ...
    {
      label: "Medium",
      color: heatMapPatterns.status.rpe.medium,
      range: "(4-6)",
    },
    { label: "High", color: heatMapPatterns.status.rpe.high, range: "(7-8)" },
    {
      label: "Highest",
      color: heatMapPatterns.status.rpe.highest,
      range: "(9-10)",
    },
  ];
    ...
    <div className="flex flex-wrap items-center justify-center gap-4 mt-4">
      ...
      {legendItems.map((item, index) => (
        <div key={index} className="flex items-center gap-1">
          <div className={`w-3 h-3 rounded-xl border ${item.color}`}></div>
...
const WeeklyHeatMapV2 = ({
  ...
  const weekStats = {
    totalWorkouts: weekDays.reduce(
      (sum, day) => sum + (day.data.workout_count || 0),
      0,
    ),
    avgRPE: (() => {
      const rpeValues = weekDays
        .filter((day) => day.data.avg_rpe)
        .map((day) => day.data.avg_rpe);
      return rpeValues.length > 0
        ? rpeValues.reduce((sum, rpe) => sum + rpe, 0) / rpeValues.length
        : null;
    })(),
    trainingDays: weekDays.filter((day) => (day.data.workout_count || 0) > 0)
      .length,
  };
...
export default WeeklyHeatMapV2;
