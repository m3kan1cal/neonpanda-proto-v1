import React, { useState } from "react";
import { useAuth } from "../contexts/AuthContext";

const GoogleIconColor = () => (
  <svg className="w-5 h-5 shrink-0" viewBox="0 0 24 24" aria-hidden="true">
    <path
      fill="#4285F4"
      d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
    />
    <path
      fill="#34A853"
      d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
    />
    <path
      fill="#FBBC05"
      d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
    />
    <path
      fill="#EA4335"
      d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
    />
  </svg>
);

const GoogleIconMono = () => (
  <svg
    className="w-5 h-5 shrink-0"
    viewBox="0 0 24 24"
    fill="currentColor"
    aria-hidden="true"
  >
    <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" />
    <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
    <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
    <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
  </svg>
);

/**
 * Social login buttons for Google OAuth.
 * mode: 'signin' | 'signup' — controls button label.
 */
const SocialLoginButtons = ({ disabled = false, mode = "signin" }) => {
  const { signInWithGoogle } = useAuth();
  const [isLoading, setIsLoading] = useState(false);

  const handleGoogleClick = async () => {
    setIsLoading(true);
    try {
      await signInWithGoogle();
      // signInWithRedirect causes a full page redirect, so setIsLoading(false)
      // will only be reached if an error is thrown before the redirect.
    } catch {
      setIsLoading(false);
    }
  };

  const label =
    mode === "signin" ? "Sign in with Google" : "Sign up with Google";

  return (
    <button
      type="button"
      onClick={handleGoogleClick}
      disabled={disabled || isLoading}
      className="group w-full flex items-center justify-center gap-3 px-6 py-3
                 bg-transparent border border-synthwave-neon-cyan text-synthwave-neon-cyan
                 rounded-full font-body font-semibold text-lg uppercase tracking-wide
                 transition-all duration-300
                 hover:bg-synthwave-neon-cyan hover:text-synthwave-bg-primary
                 focus:outline-none focus:ring-2 focus:ring-synthwave-neon-cyan/50 focus:ring-offset-2 focus:ring-offset-synthwave-bg-primary
                 cursor-pointer
                 disabled:opacity-50 disabled:cursor-not-allowed
                 disabled:hover:bg-transparent disabled:hover:text-synthwave-neon-cyan
                 min-h-[48px]"
    >
      {isLoading ? (
        <div className="w-5 h-5 shrink-0 border-2 border-current border-t-transparent rounded-full animate-spin" />
      ) : (
        <>
          <span className="block group-hover:hidden">
            <GoogleIconColor />
          </span>
          <span className="hidden group-hover:block">
            <GoogleIconMono />
          </span>
        </>
      )}
      <span className="font-body font-semibold">
        {isLoading ? "Redirecting to Google..." : label}
      </span>
    </button>
  );
};

export default SocialLoginButtons;
