...
1. **Data-first, not chart-first.** Every chart must answer a question the athlete actually asks: *"Am I getting stronger?" "Am I training balanced?" "Should I deload?"* If we can't name the question, we don't build the chart.
...
- [ ] Create `src/components/analytics/AnalyticsPage.jsx` — page layout with header, time-range selector, chart grid
...
<div className="bg-synthwave-bg-secondary border border-synthwave-text-muted/20
