...
GSI1:
PK: coach#{coachId}
SK: user#{userId}#programDesignerSession#{sessionId}
...
```
POST   /users/{userId}/coaches/{coachId}/program-designer-sessions
POST   /users/{userId}/coaches/{coachId}/program-designer-sessions/{sessionId}/send-message
GET    /users/{userId}/coaches/{coachId}/program-designer-sessions
GET    /users/{userId}/coaches/{coachId}/program-designer-sessions/{sessionId}
GET    /users/{userId}/coaches/{coachId}/program-designer-sessions/{sessionId}/config-status
```
...
- [ ] Program Designer Session entity CRUD operations
...
function CoachConversations({ userId, coachId, conversationType, sessionId }) {
  ...
  const sendMessage = async (userMessage) => {
    ...
    const endpoint =
      conversationType === "program-creation"
        ? `/users/${userId}/coaches/${coachId}/program-designer-sessions/${sessionId}/send-message`
  ...
  const configStatus =
    conversationType === "program-creation" ? useConfigStatus(sessionId) : null;

  return (
    <div className="conversation-container">
      {/* Program creation progress indicator */}
      {conversationType === "program-creation" && (
        <ProgramDesignerProgress configStatus={configStatus} />
      )}
...
- **ProgramDesignerChat**: Conversation interface for program creation
