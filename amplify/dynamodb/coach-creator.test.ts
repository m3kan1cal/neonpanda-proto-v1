...
import { updateCoachCreatorSession } from "./coach-creator";

const baseAttributes = {
  sessionId: "session_user001_111_aaa",
  userId: "user001",
  title: "Old title",
...
describe("updateCoachCreatorSession", () => {
  it("returns Date objects (not raw ISO strings) on lastActivity / startedAt", async () => {
    ...
    const result = await updateCoachCreatorSession("user001", "session_user001_111_aaa", {
      title: "New title",
    });
  ...
  it("does NOT read the existing item before writing (no read-modify-write race)", async () => {
    ...
    await updateCoachCreatorSession("user001", "session_user001_111_aaa", {
      title: "x",
    });
  ...
  it("throws 'not found' when DDB returns no Attributes", async () => {
    ...
    await expect(
      updateCoachCreatorSession("user001", "session_user001_111_aaa", {
        title: "x",
      }),
    ).rejects.toThrow(/Coach creator session not found: session_user001_111_aaa/);
  });
