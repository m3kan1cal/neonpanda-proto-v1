...
export const handler = async (event: BuildCoachConversationSummaryEvent) => {
  try {
    ...
    const conversationItem = await getCoachConversation(event.userId, event.coachId, event.conversationId);
    ...
    const existingSummaryItem = await getCoachConversationSummary(event.userId, event.conversationId);
    ...
    if (existingSummary) {
      console.info('Found existing summary to build upon:', {
        ...
        previousMessageCount: existingSummary.metadata.messageRange.totalMessages,
        confidence: existingSummary.metadata.confidence
      });
    } else {
      console.info('No existing summary found - creating first summary for this conversation');
    }

    // Build the summarization prompt
    console.info('📝 Building conversation summary prompt...');
    const summaryPrompt = buildCoachConversationSummaryPrompt(conversation, coachConfig, existingSummary);

    console.info('Generated summary prompt:', {
      promptLength: summaryPrompt.length,
      hasExistingSummary: !!existingSummary,
      messageCount: conversation.messages.length,
      promptPreview: summaryPrompt.substring(0, 1000) + (summaryPrompt.length > 1000 ? '...' : '')
    });
    ...
    const summary = parseCoachConversationSummary(aiResponse, event, conversation);
