...
import { queryMonthlyAnalytics } from '../../dynamodb/operations';
...
const baseHandler: AuthenticatedHandler = async (event) => {
  ...
  const userId = event.user.userId;
    ...
    if (sortBy && !['monthStart', 'monthEnd', 'workoutCount'].includes(sortBy)) {
      return createErrorResponse(400, 'Invalid sortBy. Must be monthStart, monthEnd, or workoutCount.');
    }
    ...
    console.info('Querying monthly reports:', {
      userId,
      options
    });
