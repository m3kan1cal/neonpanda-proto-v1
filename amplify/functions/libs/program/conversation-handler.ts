...
export async function* handleTodoListConversation(
  ...
  try {
    ...
    session.programTodoList = await extractAndUpdateTodoList(
      userResponse,
    ...
    const complete = isSessionComplete(session.programTodoList!);

    // Get progress based on to-do list
    const todoProgress = getTodoProgress(session.programTodoList!);
