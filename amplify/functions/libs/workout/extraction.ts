...
export const buildWorkoutExtractionPrompt = (userMessage: string, coachConfig: CoachConfig): string => {
  return `
CRITICAL: Return ONLY valid JSON. No markdown backticks, no explanations, no text outside JSON.
 ...
 */
export const parseAndValidateWorkoutData = (extractedData: string, userId: string): UniversalWorkoutSchema => {
  try {
    ...
    console.error('Raw extracted data:', extractedData);
