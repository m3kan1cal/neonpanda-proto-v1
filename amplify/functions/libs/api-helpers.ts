...
export function fixMalformedJson(jsonString: string): string {
  try {
    ...
    try {
      ...
      console.error("Final brace count:", {
        finalOpenBraces,
        finalCloseBraces,
      });

      // If we still have a position error, log context
      if (
        fixError instanceof SyntaxError &&
        fixError.message.includes("position")
      ) {
        const newPositionMatch = fixError.message.match(/position (\d+)/);
        if (newPositionMatch) {
          const newPosition = parseInt(newPositionMatch[1]);
          const newContextStart = Math.max(0, newPosition - 50);
          const newContextEnd = Math.min(fixed.length, newPosition + 50);
          console.error("Final error context:", {
            position: newPosition,
            contextBefore: fixed.substring(newContextStart, newPosition),
            contextAfter: fixed.substring(newPosition, newContextEnd),
          });
        }
      }

      throw new Error(
        `JSON parsing failed even after attempted fixes: ${fixError instanceof Error ? fixError.message : "Unknown error"}`
      );
    }
  }
}
