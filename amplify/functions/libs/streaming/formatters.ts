...
import { SseEvent, SseCompleteEvent, SuggestionAction } from "./types";
...
export function formatProgressEvent(
  progressType: ProgressEventType,
  message?: string,
): string {
...
export function getErrorCodeFromError(error: Error): string {
  ...
  if (message.includes("Missing required path parameters"))
    return "MISSING_PATH_PARAMS";
  ...
  if (message.includes("userResponse is required"))
    return "INVALID_USER_RESPONSE";
  if (message.includes("Message timestamp is required"))
    return "MISSING_TIMESTAMP";
  if (message.includes("Conversation not found"))
    return "CONVERSATION_NOT_FOUND";
  if (message.includes("Coach configuration not found"))
    return "COACH_CONFIG_NOT_FOUND";
...
export function formatAuthErrorEvent(error: Error): string {
  return formatErrorEvent(error.message, getErrorCodeFromError(error), error);
}
 ...
 *
 * @param suggestionType - Type of suggestion (e.g., 'program_design_mode')
 * @param message - Message to display to user
 * @param actions - Array of action buttons with handlers
 * @param confidence - AI confidence score for this suggestion (0.0 to 1.0)
 */
export function formatSuggestionEvent(
