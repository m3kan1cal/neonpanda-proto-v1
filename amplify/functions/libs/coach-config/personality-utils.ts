...
export function getCoachPersonalityContext(
  ...
  return {
    ...
    learningAdaptationPrompt: config.generated_prompts.learning_adaptation_prompt,
    ...
    experienceLevel: config.technical_config.experience_level,

    // Safety constraints
    contraindicatedExercises: config.technical_config.safety_constraints.contraindicated_exercises,
    requiredModifications: config.technical_config.safety_constraints.required_modifications,
    injuryConsiderations: config.technical_config.injury_considerations || [],

    // Methodology details
    methodologyReasoning: config.selected_methodology.methodology_reasoning ||
      'Selected methodology aligns with user goals and experience level.',
    programmingEmphasis: config.selected_methodology.programming_emphasis || 'balanced',
...
export function formatCoachPersonalityForPrompt(
  ...
  if (context) {
    sections.push(`YOU ARE ${personalityContext.coachName.toUpperCase()} - ${context.toUpperCase()}`);
  }

  // Core identity
  sections.push(`# COACH IDENTITY
You are ${personalityContext.coachName}, a ${personalityContext.specializations.join(', ')} coach.
...
## Your Expertise
Your specializations: ${personalityContext.specializations.join(', ')}
Your programming focus: ${personalityContext.programmingFocus.join(', ')}
...
${personalityContext.motivationPrompt}`);
  ...
  if (includeCriticalDirective && personalityContext.criticalTrainingDirective?.enabled) {
...
- **NEVER recommend these exercises**: ${personalityContext.contraindicatedExercises.join(', ')}
- **ALWAYS apply these modifications**: ${personalityContext.requiredModifications.join(', ')}
