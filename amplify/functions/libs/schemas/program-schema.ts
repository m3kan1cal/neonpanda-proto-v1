...
const WORKOUT_TEMPLATE_SCHEMA = {
  type: 'object',
  required: ['templateId', 'groupId', 'dayNumber', 'name', 'type', 'description', 'scoringType', 'estimatedDuration', 'restAfter'],
  properties: {
    templateId: {
      type: 'string',
      pattern: '^template_.*$',
      description: 'Unique template identifier: template_{userId}_{timestamp}_{shortId}',
    },
    groupId: {
      type: 'string',
      pattern: '^group_.*$',
      description: 'Groups templates for same day: group_{userId}_{timestamp}_{shortId}',
    },
    dayNumber: {
      type: 'number',
      minimum: 1,
    ...
    name: {
      type: 'string',
      description: 'User-facing workout name (e.g., "Strength Block", "Lower Body Burn")',
    },
    type: {
      type: 'string',
      enum: [
        'strength', 'accessory', 'conditioning', 'skill', 'mobility',
        'warmup', 'cooldown', 'recovery', 'power', 'olympic',
        'endurance', 'flexibility', 'balance', 'core', 'stability', 'mixed'
      ],
      description: 'Template type/category',
    },
    description: {
      type: 'string',
      description: 'Natural language workout description (coach-like, prescriptive)',
    },
    prescribedExercises: {
      type: 'array',
      items: { type: 'string' },
    ...
    scoringType: {
      type: 'string',
      enum: ['load', 'time', 'amrap', 'reps', 'rounds', 'distance', 'quality', 'completion', 'none'],
...
export const PHASE_SCHEMA = {
  type: 'object',
  required: ['phaseId', 'name', 'description', 'startDay', 'endDay', 'durationDays', 'focusAreas', 'workouts'],
  properties: {
    phaseId: {
      type: 'string',
      pattern: '^phase_.*$',
      description: 'Unique phase identifier: phase_{userId}_{timestamp}_{shortId}',
    },
    name: {
      type: 'string',
...
export const PHASE_STRUCTURE_SCHEMA = {
  ...
  properties: {
    phases: {
      ...
      items: {
        type: 'object',
        required: ['phaseId', 'name', 'description', 'startDay', 'endDay', 'durationDays', 'focusAreas'],
        properties: {
          phaseId: {
            type: 'string',
            pattern: '^phase_.*$',
...
export const PROGRAM_SCHEMA = {
  ...
  properties: {
    programId: {
      type: 'string',
      pattern: '^program_.*$',
      description: 'Unique program identifier: program_{userId}_{timestamp}_{shortId}',
    },
    userId: {
      type: 'string',
      pattern: '^user_.*$',
    ...
    coachIds: {
      type: 'array',
      items: { type: 'string', pattern: '^coach_.*$' },
      description: 'IDs of coaches involved in programming',
    },
    coachNames: {
      type: 'array',
      items: { type: 'string' },
