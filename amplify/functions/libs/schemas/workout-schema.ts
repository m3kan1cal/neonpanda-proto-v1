...
export const WORKOUT_SCHEMA = {
  ...
  properties: {
    workout_id: {
      type: 'string',
      pattern: '^workout_.*$',
      description: 'Unique workout identifier following pattern: workout_{userId}_{timestamp}_{shortId}',
    },
    user_id: {
      type: 'string',
      pattern: '^user_.*$',
    ...
    methodology: {
      type: ['string', 'null'],
      description: 'Training methodology used (e.g., "Westside Barbell", "Conjugate Method")',
    },
    workout_name: {
      type: ['string', 'null'],
      description: 'Name or title of the workout (e.g., "Fran", "Deadlift Day")',
    },
    workout_type: {
      type: 'string',
      enum: ['strength', 'cardio', 'flexibility', 'skill', 'competition', 'recovery', 'hybrid'],
      description: 'Primary type/focus of the workout',
    },
    duration: {
      type: ['number', 'null'],
      description: 'Total workout work time in seconds (e.g., for AMRAPs, this equals time_cap; for timed workouts, this is the actual work duration)',
    },
    session_duration: {
      type: ['number', 'null'],
      description: 'Total gym session time in seconds, including warm-up, workout, and cool-down (optional, only if user specifies total time spent)',
    },
    location: {
      type: 'string',
        ...
        heart_rate: {
          ...
          properties: {
            ...
            zones: {
              type: 'object',
              properties: {
                zone_1: { type: ['number', 'null'], description: 'Time in zone 1 (minutes)' },
                zone_2: { type: ['number', 'null'], description: 'Time in zone 2 (minutes)' },
                zone_3: { type: ['number', 'null'], description: 'Time in zone 3 (minutes)' },
                zone_4: { type: ['number', 'null'], description: 'Time in zone 4 (minutes)' },
                zone_5: { type: ['number', 'null'], description: 'Time in zone 5 (minutes)' },
              },
            },
          },
        },
      ...
      properties: {
                  ...
                  exercises: {
                    ...
                    items: {
                      ...
                      properties: {
                        ...
                        variation: {
                          type: ['string', 'null'],
                          description: 'Exercise variation (e.g., "butterfly", "kipping")',
                        },
                        assistance: {
                          type: ['string', 'null'],
                          description: 'Assistance equipment used (e.g., "bands", "box")',
                        },
                        weight: {
                          type: 'object',
                          properties: {
                        ...
                        reps: {
                          ...
                          properties: {
                            ...
                            broken_sets: {
                              type: ['array', 'null'],
                              items: { type: 'number' },
                              description: 'How reps were broken up (e.g., [10, 8, 7])',
                            },
                            rest_between_sets: {
                              type: ['array', 'null'],
                              items: { type: 'number' },
              ...
              properties: {
                ...
                score: {
                  ...
                  properties: {
                    ...
                    type: {
                      type: 'string',
                      enum: ['time', 'rounds', 'reps', 'weight', 'distance', 'points'],
        ...
        powerlifting: {
          ...
          properties: {
            session_type: {
              type: 'string',
              enum: ['max_effort', 'dynamic_effort', 'repetition_method', 'competition_prep'],
                  ...
                  sets: {
                    ...
                    items: {
                      ...
                      properties: {
                        set_type: {
                          type: 'string',
                          enum: ['opener', 'second', 'third', 'warmup', 'working', 'accessory'],
        ...
        running: {
          type: 'object',
          required: ['run_type', 'total_distance', 'distance_unit', 'total_time', 'average_pace', 'surface'],
          properties: {
            run_type: {
              type: 'string',
              enum: [
            ...
            weather: {
              ...
              properties: {
                ...
                conditions: {
                  type: ['string', 'null'],
                  enum: ['sunny', 'cloudy', 'rainy', 'snowy', 'windy', 'foggy', null],
            ...
            segments: {
              ...
              items: {
                type: 'object',
                required: ['segment_number', 'segment_type', 'distance', 'time', 'pace', 'effort_level', 'terrain'],
                properties: {
                  segment_number: {
                    type: 'integer',
                  ...
                  segment_type: {
                    type: 'string',
                    enum: ['warmup', 'working', 'interval', 'recovery', 'cooldown', 'main'],
    ...
    pr_achievements: {
      ...
      items: {
        type: 'object',
        required: ['exercise', 'discipline', 'pr_type', 'new_best', 'significance'],
        properties: {
          exercise: {
            type: 'string',
          ...
          pr_type: {
            type: 'string',
            enum: ['workout_time', '1rm', 'volume_pr', 'distance_pr', 'pace_pr'],
    ...
    subjective_feedback: {
      ...
      properties: {
        ...
        mental_state: {
          type: ['string', 'null'],
          enum: ['focused', 'distracted', 'motivated', 'tired', 'energetic', 'stressed', 'calm', null],
    ...
    metadata: {
      ...
      properties: {
        logged_via: {
          type: 'string',
          enum: ['conversation', 'app', 'wearable', 'manual'],
        ...
        generation_method: {
          type: 'string',
          enum: ['tool', 'fallback'],
          description: 'Method used to generate workout data (tool-based or fallback prompt-based)',
        },
...
export function validateWorkout(workout: any): {
  ...
  const validTypes = ['strength', 'cardio', 'flexibility', 'skill', 'competition', 'recovery', 'hybrid'];
  ...
  if (!workout.metadata) {
    ...
    if (workout.metadata.schema_version && workout.metadata.schema_version !== '2.0') {
