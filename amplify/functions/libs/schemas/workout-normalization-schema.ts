/**
 * Workout Normalization Response Schema
 *
...
export const NORMALIZATION_RESPONSE_SCHEMA = {
  ...
  properties: {
    isValid: {
      type: 'boolean',
      description: 'Whether the workout data is valid after normalization. Set to TRUE if: (1) no issues found, OR (2) all issues were corrected. Set to FALSE only if critical issues exist that could NOT be corrected.'
    },
    normalizedData: WORKOUT_SCHEMA,
    issues: {
      type: 'array',
      ...
      items: {
        ...
        properties: {
          type: {
            type: 'string',
            enum: ['structure', 'data_quality', 'cross_reference'],
